...
// 데이터 가져오기
$sql = "SELECT year, value FROM country_china ORDER BY year ASC";
...
while ($row = mysqli_fetch_assoc($result)) {
    if ($row['year'] == 2021) {
        ...
        $row['annotationText'] = 'https://www.bbc.com/korean/international-59304371';
    } else {
        $row['annotation'] = null;
...
<script type="text/javascript">
    ...
    function drawChart() {
        ...
        data.addColumn('number', 'China Population');
        data.addColumn({
            type: 'string',
            role: 'annotation'
        });
