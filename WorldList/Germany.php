...
// 데이터 가져오기
$sql = "SELECT year, value FROM country_germany ORDER BY year ASC";
$result = mysqli_query($conn, $sql);

$chart_data = array();
while ($row = mysqli_fetch_assoc($result)) {
    if ($row['year'] == 1990) {
        $row['annotation'] = '독일의 재통일';
        $row['annotationText'] = 'https://www.unikorea.go.kr/unikorea/news/card/?boardId=bbs_0000000000000002&mode=view&cntId=54483&category=&pageIdx=5';
    } else {
        $row['annotation'] = null;
...
<script type="text/javascript">
    ...
    function drawChart() {
        ...
        data.addColumn('number', 'Germany Population');
        data.addColumn({
            type: 'string',
            role: 'annotation'
        });
